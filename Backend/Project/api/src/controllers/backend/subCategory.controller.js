...
exports.create = async (request, response) => {
    ...
    if(request.body != undefined){
        if(request.body.name != undefined || request.body.name != ''){
            ...
            saveData.slug = await generateUniqueSlug(subCategoryModal, slug)
        }
    }

    await subCategoryModal(saveData).save()
    .then((result) => {
        const data = {
            _status : true,
...
exports.view = async(request, response) => {
    ...
    var total_records = await subCategoryModal.find(filter).countDocuments();

    await subCategoryModal.find(filter).limit(limit).skip(skip)
...
exports.details = async(request, response) => {
    await subCategoryModal.findOne({
        _id : request.params.id,
        deleted_at : null
    })
    .then((result) => {
