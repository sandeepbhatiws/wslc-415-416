...
exports.create = async (request, response) => {
    ...
    if(request.body != undefined){
        if(request.body.name != undefined || request.body.name != ''){
            ...
            saveData.slug = await generateUniqueSlug(subSubCategoryModal, slug)
        }
    }

    await subSubCategoryModal(saveData).save()
    .then((result) => {
        const data = {
            _status : true,
...
exports.view = async(request, response) => {
    ...
    var total_records = await subSubCategoryModal.find(filter).countDocuments();

    await subSubCategoryModal.find(filter).limit(limit).skip(skip)
...
exports.details = async(request, response) => {
    await subSubCategoryModal.findOne({
        _id : request.params.id,
        deleted_at : null
    })
    .then((result) => {
